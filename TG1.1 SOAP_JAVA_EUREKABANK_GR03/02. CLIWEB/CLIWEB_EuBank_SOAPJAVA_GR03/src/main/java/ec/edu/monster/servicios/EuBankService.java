...
public class EuBankService {
    ...
    // 3.5 Traer Todos los Movimientos
    public List<MovimientoData> traerTodosMovimientos() {
        List<DatosCuenta> cuentas = port.traerCuentasConClientes();
        java.util.ArrayList<MovimientoData> todos = new java.util.ArrayList<>();
        if (cuentas != null) {
            for (DatosCuenta c : cuentas) {
                List<MovimientoData> movs = port.traerMovimientos(c.getCodigo());
                if (movs != null) {
                    todos.addAll(movs);
                }
            }
