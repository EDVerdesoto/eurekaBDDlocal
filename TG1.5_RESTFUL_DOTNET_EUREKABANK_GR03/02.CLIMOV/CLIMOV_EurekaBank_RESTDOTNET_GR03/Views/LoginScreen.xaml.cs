...
namespace CLIMOV_EurekaBank_RESTDOTNET_GR03.Views
{
    public partial class LoginScreen : ContentPage
    {
        ...
        public LoginScreen()
        {
            ...
            _loading = new ActivityIndicator { Color = Color.FromArgb("#7C3AED"), IsVisible = false };
                    ...
                    Children =
                    {
                        new VerticalStackLayout
                        {
                            ...
                            Children =
                            {
                                ...
                                new Label { Text = "Sistema de Gestión Bancaria", FontSize = 18, FontAttributes = FontAttributes.Bold, TextColor = Color.FromArgb("#7C3AED"), HorizontalTextAlignment = TextAlignment.Center, Margin = new Thickness(0, 0, 0, 14) },
                                Field(_usuarioEntry),
                                Field(_claveEntry),
                                _loginButton,
                                _loading,
        ...
        private async void OnLoginClicked(object? sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (!ok)
                {
                    var lastErr = EurekaBankClient.LastConnectionError;
                    if (lastErr != null && !lastErr.Contains("Credenciales"))
                        ShowMessage($"Error de conexion: {lastErr}", false);
                    else
